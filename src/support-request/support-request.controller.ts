...
export class SupportRequestController {
  ...
  async getSupportRequestsByUser(    @Query('limit') limit: number = 10,
                                     @Query('offset') offset: number = 0,
                                     @Query('isActive') isActive: boolean = true,
                                     @Req() request: any) {
    return this.supportRequestService.findSupportRequests({user:  request.user.userId, isActive });
  }
  @Get('manager/support-requests')
  @UseGuards(AuthGuard, RoleGuard)
  @Roles(RoleEnum.client)
  @UseInterceptors(SupportRequestInterceptorForManager)
  async getSupportRequestsManagerByUser(    @Query('limit') limit: number = 10,
                                            @Query('userId') userId: string,
                                     @Query('offset') offset: number = 0,
                                     @Query('isActive') isActive: boolean = true,
                                     @Req() request: any) {
    return await this.supportRequestService.findSupportRequestsManager({user:  userId, isActive });
  }
  ...
  async createSupportRequest(@Body() supportRequestDto: CreateSupportRequestDto, @Req() request: any) {
    return await this.supportRequestClientService.createSupportRequest({ user: request.user.userId, text: supportRequestDto.text});
  }
